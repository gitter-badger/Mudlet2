#ifndef _TLUA_INTERPRETER_H
#define _TLUA_INTERPRETER_H
/***************************************************************************
...
public:
    ...
    void run()
    {
        qDebug("TLuaMainThread::run() called. Initializing Gatekeeper...");
        ...
        qDebug("TLuaMainThread::run() done exit.");
    }
    ...
    {
        qDebug("TLuaInterpreter::TLuaMainThread::postJob(...) Posting new job:\n<%s>", qPrintable( code ));
        std::string job = code.toLatin1().data();
        mutex.lock();
        mJobQueue.push(job);
        mutex.unlock();
        qDebug("TLuaInterpreter::TLuaMainThread::postJob(...) Posting new job - Done.");
    }
