...
// build (if present, that value will be nil in release version so can be used
// as a test for release version!)
//
...
//
// Introduced in 3.0.1-rc2, to assist package writers to allow for new, or
// improvements to, built-in LUA functions.
int TLuaInterpreter::getMudletVersion( lua_State * L )
{
    ...
    if( versionData.size() != 3 )
    {
        qWarning("TLuaInterpreter::getMudletVersion(): ERROR: Version data not correctly set on compilation,\n"
                 "   is the VERS""ION value in the project file present?");
        // Split in the above string required to avoid C++ compiler preprocessor
        // macro replacement 8-)
    ...
    if( ! ok )
    {
        qWarning("TLuaInterpreter::getMudletVersion(): ERROR: Version data not correctly parsed,\n"
                 "   was the VERS""ION value in the project file correct set at compilation time?");
        // Split in the above string required to avoid C++ compiler preprocessor
        // macro replacement 8-)
        lua_pushstring( L, "getMudletVersion: sorry, version information not available." );
        lua_error( L );
